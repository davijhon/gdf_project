    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if option == "1":
                ...
                messages.error(
                    self.request,
                    "Aún no posee ningun proceso de Gestion de Facilidad configurado.",
                )
...
class InitialConfView(LoginRequiredMixin, View):
    ...
    def post(self, *args, **kwargs):
        ...
        try:
            # Set "Facilidad" name.
            if (
                comunication_form.is_valid()
                and logistic_formset.is_valid()
                and vehiculo_form.is_valid()
                and gpo_elect_form.is_valid()
            ):
                ...
                with transaction.atomic():
                    comunicaciones_option = cd_comunication.get(
                        "comunicaciones_options", None
                    )
                    ...
                    facilidad = Facilidad.objects.create(nombre=fac_name.nombre)
                    ...
                    eqcoms = [
                        EqCom.objects.create(nombre=i["nombre"]) for i in cd_logistic
                    ]

                    for e in eqcoms:
                        Equipamiento.objects.create(
                            content_object=e, facilidad=facilidad
                        )

                    # Vehiculo # TODO UNDRY
                    v = Vehiculo.objects.create(
                        nombre_vehiculo=cd_vehiculo.get("nombre_vehiculo"),
                        cant_combustible_vehiculo=cd_vehiculo.get(
                            "cant_combustible_vehiculo"
                        ),
                        cap_combustible_vehiculo=cd_vehiculo.get(
                    ...
                    ge = GpoElectr.objects.create(
                        ...
                        cant_combustible_gpo_elect=cd_gpo_elect.get(
                            "cant_combustible_gpo_elect"
                        ),
                        cap_combustible_gpo_elect=cd_gpo_elect.get(
                    ...
                    messages.success(
                        self.request,
                        "Se ha configurado la Facilidad satisfactoriamente.",
                    )
                    data = {
                        "status": 200,
                        "success_url": reverse_lazy(
                            "ecr:ecr_index", kwargs={"pk": facilidad.pk}
                        ),
                        "message": "",
                    }
...
class EcrDashboardView(LoginRequiredMixin, DetailView):
    ...
    def dispatch(self, *args, **kwargs):
        ...
        if not self.request.user.facilidad:
            messages.error(
                self.request,
                "No poseé o no se encuentra en algún proceso de Facilidad.",
            )
            return redirect("ecr:ecr_action_option")
        ...
        try:
            ...
            self.vehiculo_obj = veh_content.get_object_for_this_type(
                id=vehiculo.object_id
            )
        except Exception as e:
        ...
        try:
            ...
            self.gpoelectr_obj = gpoelectr_content.get_object_for_this_type(
                id=gpoelectr.object_id
            )
        except Exception as e:
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["eqcom_list"] = EqCom.objects.filter(
            id__in=eqcom_list.values_list("object_id")
        )
            ...
            with transaction.atomic():
                if vehiculo:
                    ...
                    if form_vehiculo.is_valid():
                        form_vehiculo.save()
                        return JsonResponse(
                            {"status": 200, "form_name": "vehiculo_form"}
                        )
                    else:
                        data = {"status": 400, "errors": str(form_vehiculo.errors)}
                ...
                if gpoelect:
                    ...
                    if form_gpoelectr.is_valid():
                        form_gpoelectr.save()
                        return JsonResponse(
                            {"status": 200, "form_name": "gpoelect_form"}
                        )
                    else:
                        data = {"status": 400, "errors": str(form_vehiculo.errors)}
                return JsonResponse(data)
        ...
        try:
            with transaction.atomic():
                if form.is_valid():
                    ...
                    facilidad_obj = Facilidad.objects.get(
                        id=self.request.POST.get("id_facilidad_obj", None)
                    )
                    ...
                    return JsonResponse(
                        {
                            "status": 200,
                            "message": "Evento generado. Visualice el Libro de Guardia.",
                        }
                    )
...
def change_status_facilidad(request):
    ...
    if request.method == "POST":
        ...
        return JsonResponse(
            {"error": "Ha ocurrido un error en la solicitud."}, status=404
        )
    return JsonResponse(
        {"error": "Ha ocurrido un problema en la actualizacion del estado."}, status=500
    )
